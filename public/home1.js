...
function render(state){
    ...
    return state.map(todo => {
        if(todo==null){
            ...
            const classString1 = `class="float"`
            ...
            return `<li data-todo = "${id}" ${classString}
                ...
                <span data-todo = "${id}"> 
                    <button data-todo = "${id}" ${classString1} id="delete" onclick = "RemoveList(this) "> X </button> 
                </span>
                <span data-todo = "${id}"> 
                    <button data-todo = "${id}" ${classString1} id="edit" onclick = "Edit(this) "> Edit </button> 
                </span>
            <li>`
        }
    })
